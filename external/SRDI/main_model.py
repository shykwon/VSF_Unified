...
class CSDI_base(nn.Module):
    def __init__(self, target_dim, config, device):
        ...
        self.diffmodel = diff_CSDI(config_diff, target_dim, device, input_dim)
    ...
    def calc_loss_valid(
        ...
        for t in range(self.num_steps):  # calculate loss for all t
            loss,loss_d= self.calc_loss(
                observed_data, cond_mask, observed_mask, side_info, is_train, set_t=t
            )
            loss_sum += loss.detach()
        return loss_sum / self.num_steps, loss_d
    def calc_loss(
        ...
        predicted,loss_d= self.diffmodel(total_input, side_info, t)  # (B,K,L)->(1,K,L)
    ...
    def forward(self, batch, is_train=0):
        (
            observed_data,
            observed_mask,
            observed_tp,
        ...
        x,loss_d= loss_func(observed_data, cond_mask, observed_mask, side_info, is_train)
        x = torch.tensor(x, requires_grad=True)
        loss_d = torch.tensor(loss_d, requires_grad=True).to(self.device)
        return x,loss_d
...
class CSDI_vsf(CSDI_base):
    def __init__(self, config, device, target_dim=140):  # target_dim就是变量个数
        super(CSDI_vsf, self).__init__(target_dim, config, device)
...
class CSDI_Forecasting(CSDI_base):
    ...
    def forward(self, batch, is_train=0):
        ...
        loss_func = self.calc_loss if is_train == 1 else self.calc_loss_valid
        loss,loss_d= loss_func(observed_data, cond_mask, observed_mask, side_info, is_train)
        loss = loss.detach().requires_grad_(True)
        loss_d = loss_d.detach().requires_grad_(True)
        return loss,loss_d
